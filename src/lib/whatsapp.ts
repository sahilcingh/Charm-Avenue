 ...
 */
export function buildWhatsAppEnquiryMessage(items: WhatsAppEnquiryItem[], discountTotal = 0): string {
    if (items.length === 0) {
    ...
    return ["Hi! I'd like to enquire about:", ...lines, ...discountLine, `Total: ₹${itemsTotal - discountTotal}`].join('\n');
}
