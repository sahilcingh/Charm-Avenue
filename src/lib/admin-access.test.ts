...
describe('decideAdminAccess', () => {
  it('lets a request through untouched on a non-admin path', () => {
    expect(decideAdminAccess({ pathname: '/shop', isLoggedIn: false, isAdmin: false })).toEqual({
      action: 'next',
    });
  });
  ...
  it('redirects a logged-out visitor away from a protected admin route to the single unified login page, remembering where they were headed', () => {
    expect(
      decideAdminAccess({ pathname: '/admin/products', isLoggedIn: false, isAdmin: false })
    ).toEqual({
      action: 'redirect',
  ...
  it('redirects a logged-in non-admin customer away from a protected admin route (the core security fix)', () => {
    expect(
      decideAdminAccess({ pathname: '/admin/products', isLoggedIn: true, isAdmin: false })
    ).toEqual({
      action: 'redirect',
      redirectTo: '/',
    });
  ...
  it('lets a logged-in admin through to a protected admin route', () => {
    expect(
      decideAdminAccess({ pathname: '/admin/products', isLoggedIn: true, isAdmin: true })
    ).toEqual({ action: 'next' });
  });

  it('treats a nested admin sub-route the same as the top-level one', () => {
    expect(
      decideAdminAccess({ pathname: '/admin/products/123', isLoggedIn: true, isAdmin: false })
    ).toEqual({
      action: 'redirect',
      redirectTo: '/',
    });
  ...
  it('encodes special characters in the remembered next path (failure case: an unencoded next could break the query string)', () => {
    expect(
      decideAdminAccess({
        pathname: '/admin/products?tab=drafts',
        isLoggedIn: false,
        isAdmin: false,
      })
    ).toEqual({
