...
import { compressImageFile, COMPRESSION_STEPS, COMPRESSION_TARGET_BYTES } from './compress-product-image';
...
describe('compressImageFile', () => {
    ...
    it('falls back to the last (most compressed) attempt if no step gets under the target (edge case: an unusually dense photo)', async () => {
        ...
        const attemptStep = vi.fn().mockImplementation(async () => blobOfSize(COMPRESSION_TARGET_BYTES + 1));
