...
interface CartContextValue {
  ...
  /** Empties every line at once — used once an enquiry/order has actually been placed. */
  clearCart: () => void;
}
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  // Derived key, not the `lines` array itself — the self-heal effect below should
  // only re-run when the SET of product ids actually changes, not on every quantity tick.
  ...
  const value = useMemo(
    ...
    [lines, itemCount, hydrated, addToCart, removeFromCart, setQuantity, adjustQuantity, clearCart]
  );
