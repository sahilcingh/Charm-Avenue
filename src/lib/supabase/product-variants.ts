...
export const STOCK_STATUS_LABELS: Record<ProductStockStatus, { label: string; color: string; bg: string }> = {
...
export function formatVariantLabel(variant: { color: string | null; size: string | null }): string | null {
...
export function resolveVariantDisplay(
    ...
    if (!variant) {
        return { price: base.price, originalPrice: base.originalPrice, image: base.image, stockStatus: null, stockCount: null };
    }
    return {
