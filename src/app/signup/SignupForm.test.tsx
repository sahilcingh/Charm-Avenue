...
describe('SignupForm', () => {
  ...
  it('shows a friendly error for a genuine unexpected failure (failure case, unrelated to account enumeration)', async () => {
    signUpMock.mockResolvedValue({
      data: { session: null },
      error: { message: 'Some unrelated backend error' },
    });
    const user = userEvent.setup();
    render(<SignupForm />);
  ...
  it('shows a friendly error for a wrong or expired code (failure case)', async () => {
    ...
    expect(
      await screen.findByText('That code is incorrect or has expired. Please request a new one.')
    ).toBeInTheDocument();
    expect(pushMock).not.toHaveBeenCalled();
  });
  ...
  it('resends the code and toasts confirmation', async () => {
    ...
    await waitFor(() =>
      expect(resendMock).toHaveBeenCalledWith({ type: 'signup', email: 'nandini@charmavenue.in' })
    );
