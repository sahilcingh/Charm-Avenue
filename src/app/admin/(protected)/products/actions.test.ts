...
import { createProduct, updateProduct, deleteProduct, addProductImage, removeProductImage, reorderProductImage, addVariant, updateVariant, removeVariant } from './actions';
        ...
        from: (table: string) => {
            ...
            if (table === 'product_images') {
                return {
                    ...
                    update: (values: unknown) => ({ eq: (col: string, val: unknown) => productImagesUpdateEqMock(values, col, val) }),
                    select: () => ({ eq: () => ({ order: () => productImagesOrderedListMock() }) }),
                };
            }
            if (table === 'product_categories') {
                return { insert: productCategoriesInsertMock, delete: () => ({ eq: productCategoriesDeleteEqMock }) };
            }
            ...
            if (table === 'product_variants') {
                return {
                    ...
                    update: (values: unknown) => ({ eq: (col: string, val: unknown) => productVariantsUpdateEqMock(values, col, val) }),
                };
            }
            // 'products'
            return {
...
describe('createProduct — Stock & Availability section', () => {
    it('saves stock_status and low_stock_threshold when tracking is turned on', async () => {
        ...
        expect(productsInsertMock).toHaveBeenCalledWith(
            expect.objectContaining({ stock_status: 'in_stock', low_stock_threshold: 2, made_to_order_lead_time: null })
        );
    });
    ...
    it('only saves made_to_order_lead_time when stock_status is made_to_order', async () => {
        ...
        expect(productsInsertMock).toHaveBeenCalledWith(expect.objectContaining({ made_to_order_lead_time: null }));
    });
    ...
    it('saves made_to_order_lead_time when stock_status is made_to_order', async () => {
        ...
        expect(productsInsertMock).toHaveBeenCalledWith(expect.objectContaining({ made_to_order_lead_time: 'Ships in 5-7 days' }));
    });
    ...
    it('leaves low_stock_threshold null when tracking is on but the field is left blank', async () => {
        ...
        expect(productsInsertMock).toHaveBeenCalledWith(expect.objectContaining({ low_stock_threshold: null }));
    });
...
describe('createProduct — Sale Window section', () => {
    it('saves the sale window as an inclusive full-day range when scheduling is turned on', async () => {
        ...
        expect(productsInsertMock).toHaveBeenCalledWith(
            expect.objectContaining({ sale_starts_at: '2026-08-01T00:00:00.000Z', sale_ends_at: '2026-08-15T23:59:59.999Z' })
        );
    });
});
...
describe('createProduct — More Details section', () => {
    it('trims and saves dimensions/material/care_instructions when provided', async () => {
        ...
        expect(productsInsertMock).toHaveBeenCalledWith(
            expect.objectContaining({ dimensions: '5cm x 3cm', material: 'Sterling silver', care_instructions: 'Keep dry' })
        );
    });
});
...
describe('createProduct — Personalization section', () => {
    ...
    it('falls back to a 50-character cap when enabled but max length is left blank', async () => {
        ...
        expect(productsInsertMock).toHaveBeenCalledWith(expect.objectContaining({ personalization_max_length: 50 }));
    });
...
describe('addProductImage', () => {
    ...
    it('uploads the file and inserts a product_images row for the given product', async () => {
        ...
        expect(productImagesInsertMock).toHaveBeenCalledWith(
            expect.objectContaining({ product_id: 'product-1', url: 'https://example.com/photo.jpg', alt: 'Side angle' })
        );
    });
});
...
describe('createProduct — Categories & Tags sync', () => {
    it('adds the primary category to product_categories, with no extra categories or tags by default', async () => {
        ...
        expect(productCategoriesInsertMock).toHaveBeenCalledWith([{ product_id: 'new-product-id', category_slug: 'accessories' }]);
...
describe('updateProduct — Categories & Tags sync', () => {
    it('resyncs by deleting existing rows first, then inserting the current selection', async () => {
        ...
        expect(productCategoriesInsertMock).toHaveBeenCalledWith([{ product_id: 'p1', category_slug: 'accessories' }]);
        expect(productTagsInsertMock).toHaveBeenCalledWith([{ product_id: 'p1', tag_slug: 'bestseller' }]);
    });
});
...
describe('updateVariant', () => {
    ...
    it('stores null instead of an invalid stock_status (defense in depth)', async () => {
        ...
        expect(productVariantsUpdateEqMock).toHaveBeenCalledWith(expect.objectContaining({ stock_status: null }), 'id', 'variant-1');
    });
    ...
    it('defaults is_active to true when not explicitly turned off', async () => {
        mockAdmin();
        await updateVariant('variant-1', 'product-1', new FormData());
        expect(productVariantsUpdateEqMock).toHaveBeenCalledWith(expect.objectContaining({ is_active: true }), 'id', 'variant-1');
    });
    ...
    it('retires the variant when isActive is explicitly set to off', async () => {
        ...
        fd.set('isActive', 'off');
        await updateVariant('variant-1', 'product-1', fd);
        expect(productVariantsUpdateEqMock).toHaveBeenCalledWith(expect.objectContaining({ is_active: false }), 'id', 'variant-1');
    });
});
