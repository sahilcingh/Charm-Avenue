...
const VALID_STOCK_STATUSES: ProductStockStatus[] = ['in_stock', 'out_of_stock', 'made_to_order', 'discontinued'];
...
function parseForm(formData: FormData): ProductFormValues {
    ...
    const stockStatus = trackStock && VALID_STOCK_STATUSES.includes(stockStatusRaw as ProductStockStatus)
        ? (stockStatusRaw as ProductStockStatus)
        : null;
    const madeToOrderLeadTime = trackStock && stockStatus === 'made_to_order'
        ? (String(formData.get('madeToOrderLeadTime') || '').trim() || null)
        : null;
    const lowStockThresholdRaw = trackStock ? String(formData.get('lowStockThreshold') || '').trim() : '';
    ...
    const personalizationMaxLengthRaw = personalizationEnabled ? String(formData.get('personalizationMaxLength') || '').trim() : '';
    ...
    return {
        ...
        personalizationLabel: personalizationEnabled ? (String(formData.get('personalizationLabel') || '').trim() || null) : null,
        personalizationRequired: personalizationEnabled && formData.get('personalizationRequired') === 'on',
        personalizationMaxLength: !personalizationEnabled
            ? null
            : (personalizationMaxLengthRaw !== '' ? Number(personalizationMaxLengthRaw) : PERSONALIZATION_MAX_LENGTH_FALLBACK),
...
async function syncProductCategoriesAndTags(
    ...
    const { error: categoriesDeleteError } = await supabase.from('product_categories').delete().eq('product_id', productId);
    ...
    if (allCategorySlugs.length > 0) {
        ...
        if (categoriesInsertError) console.error('Failed to insert product_categories:', categoriesInsertError.message);
    }

    const { error: tagsDeleteError } = await supabase.from('product_tags').delete().eq('product_id', productId);
    if (tagsDeleteError) {
...
export async function createProduct(formData: FormData) {
    ...
    if (inserted?.id) {
        await syncProductCategoriesAndTags(supabase, inserted.id, values.categorySlug, values.extraCategorySlugs, values.tagSlugs);
    }
...
export async function updateProduct(productId: string, formData: FormData) {
    ...
    await syncProductCategoriesAndTags(supabase, productId, values.categorySlug, values.extraCategorySlugs, values.tagSlugs);
...
export async function reorderProductImage(imageId: string, productId: string, direction: 'up' | 'down') {
    ...
    await Promise.all([
        supabase.from('product_images').update({ sort_order: swapWith.sort_order }).eq('id', current.id),
        supabase.from('product_images').update({ sort_order: current.sort_order }).eq('id', swapWith.id),
    ]);
...
export async function updateVariant(variantId: string, productId: string, formData: FormData) {
    ...
    const update: Record<string, unknown> = {
        ...
        original_price_override: originalPriceOverrideRaw !== '' ? Number(originalPriceOverrideRaw) : null,
        stock_status: VALID_STOCK_STATUSES.includes(stockStatusRaw as ProductStockStatus) ? stockStatusRaw : null,
