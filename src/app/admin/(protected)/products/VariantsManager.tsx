...
function VariantRow({ variant, productId }: { variant: DbProductVariant; productId: string }) {
    ...
    const [stockStatus, setStockStatus] = useState<'' | ProductStockStatus>(variant.stock_status ?? '');
    ...
    return (
        <div className="flex flex-wrap items-end gap-3 p-3 rounded-2xl border" style={{ borderColor: 'var(--blush-border)' }}>
            <button
                type="button"
            ...
            <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => handleImagePick(e.target.files)} />

            <div className="w-24">
                <label className={fieldLabelClass} style={{ color: 'var(--blush-muted)' }}>Color</label>
                <input value={color} onChange={(e) => { setColor(e.target.value); setDirty(true); }} className={fieldClass} style={{ color: 'var(--blush-text)' }} placeholder="Red" />
            </div>
            <div className="w-20">
                <label className={fieldLabelClass} style={{ color: 'var(--blush-muted)' }}>Size</label>
                <input value={size} onChange={(e) => { setSize(e.target.value); setDirty(true); }} className={fieldClass} style={{ color: 'var(--blush-text)' }} placeholder="M" />
            </div>
            <div className="w-24">
                <label className={fieldLabelClass} style={{ color: 'var(--blush-muted)' }}>Price (₹)</label>
                <input type="number" value={priceOverride} onChange={(e) => { setPriceOverride(e.target.value); setDirty(true); }} className={fieldClass} style={{ color: 'var(--blush-text)' }} placeholder="Same" />
            </div>
            <div className="w-36">
                <label className={fieldLabelClass} style={{ color: 'var(--blush-muted)' }}>Stock</label>
                <select value={stockStatus} onChange={(e) => { setStockStatus(e.target.value as '' | ProductStockStatus); setDirty(true); }} className={fieldClass} style={{ color: 'var(--blush-text)' }}>
                    {STOCK_OPTIONS.map((o) => (
            ...
            <div className="w-20">
                <label className={fieldLabelClass} style={{ color: 'var(--blush-muted)' }}>Count</label>
                <input type="number" value={stockCount} onChange={(e) => { setStockCount(e.target.value); setDirty(true); }} className={fieldClass} style={{ color: 'var(--blush-text)' }} placeholder="—" />
            </div>

            <label className="flex flex-col items-center gap-1 cursor-pointer">
                <span className={fieldLabelClass} style={{ color: 'var(--blush-muted)' }}>Active</span>
                <span
            ...
            <div className="flex items-center gap-1.5 ml-auto">
                ...
                {confirmingDelete ? (
                    <div className="flex items-center gap-1.5">
                        ...
                        <button type="button" onClick={() => setConfirmingDelete(false)} aria-label="Cancel delete" className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: 'var(--blush-bg)', color: 'var(--blush-muted)' }}>
            ...
            {error && <p className="text-xs font-medium w-full" style={{ color: 'var(--blush-rose-dark)' }}>{error}</p>}
        </div>
    );
}

export default function VariantsManager({ productId, variants }: { productId: string; variants: DbProductVariant[] }) {
    const [isPending, startTransition] = useTransition();
    const [error, setError] = useState<string | null>(null);
    ...
    return (
        <div className="flex flex-col gap-3">
            ...
            >
                <Icon name={isPending ? 'ArrowPathIcon' : 'PlusIcon'} size={16} className={isPending ? 'animate-spin' : ''} />
