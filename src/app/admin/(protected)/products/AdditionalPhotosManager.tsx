...
export default function AdditionalPhotosManager({ productId, images }: { productId: string; images: DbProductImage[] }) {
        ...
        <div className="flex flex-col gap-3">
            {images.length > 0 && (
                <div className="flex flex-wrap gap-2">
                    {images.map((img, i) => (
                        <div key={img.id} className="relative w-16 h-16 rounded-2xl overflow-hidden shrink-0 group">
                            ...
                            >
                                <div className="flex gap-1">
                                    <button
                                        ...
                                        onClick={() => startTransition(() => reorderProductImage(img.id, productId, 'up'))}
                                    ...
                                    <button
                                        ...
                                        onClick={() => startTransition(() => reorderProductImage(img.id, productId, 'down'))}
            ...
            <input ref={fileInputRef} type="file" accept="image/*" onChange={(e) => handleFile(e.target.files)} className="hidden" />
        </div>
    );
}
