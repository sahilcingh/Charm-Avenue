  ...
  return (
    <div className={`bg-white rounded-3xl p-6 md:p-7 card-bubble animate-enter ${delay}`}>
      <div className="flex items-center gap-2.5 mb-5">
        ...
        <div>
          <h2 className="font-bold text-sm" style={{ color: 'var(--blush-text)' }}>
            {title}
          </h2>
          {subtitle && (
...
export default function ProductForm({
  ...
  const [saleWindowOpen, setSaleWindowOpen] = useState(
    Boolean(product?.sale_starts_at || product?.sale_ends_at)
  );
  const [saleStartsAt, setSaleStartsAt] = useState(
    product?.sale_starts_at ? product.sale_starts_at.slice(0, 10) : ''
  );
  const [saleEndsAt, setSaleEndsAt] = useState(
    product?.sale_ends_at ? product.sale_ends_at.slice(0, 10) : ''
  );
  ...
  const [madeToOrderLeadTime, setMadeToOrderLeadTime] = useState(
    product?.made_to_order_lead_time ?? ''
  );
  ...
  const [personalizationEnabled, setPersonalizationEnabled] = useState(
    product?.personalization_enabled ?? false
  );
  ...
  const [personalizationRequired, setPersonalizationRequired] = useState(
    product?.personalization_required ?? false
  );
  const [personalizationMaxLength, setPersonalizationMaxLength] = useState(
    product?.personalization_max_length?.toString() ?? ''
  );
  ...
  const [categoriesTagsOpen, setCategoriesTagsOpen] = useState(
    selectedCategorySlugs.length > 0 || selectedTagSlugs.length > 0
  );
  const [extraCategories, setExtraCategories] = useState<Set<string>>(
                        ...
                        return (
                          ...
                          >
                            <input
                              ...
                              checked={checked}
                              onChange={() =>
                                setExtraCategories((prev) => toggleSetValue(prev, c.slug))
                              }
                              className="sr-only"
                            />
                            {c.emoji} {c.title}
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            checked={checked}
                            onChange={() =>
                              setTagSelections((prev) => toggleSetValue(prev, t.slug))
                            }
                            className="sr-only"
                          />
                          {t.label}
          ...
          {!trackStock ? (
            ...
            <>
              ...
              <div>
                ...
                />
                <p className="text-xs mt-1.5" style={{ color: 'var(--blush-muted)' }}>
                  Shown to shoppers as &quot;only {lowStockThreshold || '2'} left&quot; once stock
                  count drops to this number or below.
                </p>
              </div>
        ...
        >
          <label className="flex items-center justify-between cursor-pointer">
            ...
            <span
              ...
              style={{
                background: personalizationEnabled ? 'var(--blush-rose)' : 'var(--blush-border)',
              }}
            >
              <span
                className="inline-block h-5 w-5 rounded-full bg-white shadow transition-transform duration-300"
                style={{
                  transform: personalizationEnabled ? 'translateX(22px)' : 'translateX(4px)',
                }}
              />
              <input
                type="checkbox"
              ...
              <div className="grid sm:grid-cols-2 gap-4 items-end">
                ...
                <label className="flex items-center gap-2 cursor-pointer pb-3">
                  <input
                    ...
                    className="w-4 h-4"
                  />
                  <span className="text-sm font-medium" style={{ color: 'var(--blush-text)' }}>
                    Required before adding to bag
                  </span>
                </label>
              </div>
            </>
          ...
          <div className="relative aspect-square" style={{ background: 'var(--blush-bg)' }}>
            ...
            {tagStyle !== 'none' && tagLabel && (
              <span
                ...
                style={{
                  background: activeTagStyle.tagBg ?? undefined,
                  color: activeTagStyle.tagText ?? undefined,
                }}
              >
                {tagLabel}
              </span>
            ...
            {!isActive && (
              ...
              >
                <span
                  className="badge-pill bg-white text-xs"
                  style={{ color: 'var(--blush-text)' }}
                >
                  Hidden
                </span>
