...
export default async function AdminOrderDetailPage({
  ...
  return (
    <div>
      {/* Picks up a status change made from another tab/device without a manual reload. */}
      <AutoRefresh />
      <Link
        href="/admin/orders"
      ...
      <div className="bg-white rounded-3xl p-6 md:p-8 card-bubble flex flex-col gap-6 animate-enter delay-200">
            ...
            >
              <span>
                ...
                {item.personalization_text && (
                  <span className="block text-xs italic" style={{ color: 'var(--blush-muted)' }}>
        ...
        <div
          className={`flex items-center justify-between ${order.discount_total > 0 ? '' : 'pt-4 border-t'}`}
          style={{ borderColor: 'var(--blush-border)' }}
        >
