...
export default async function AdminOrdersPage({
    ...
    const pendingCount = all.filter((o) => o.status === 'pending_whatsapp' || o.status === 'pending_payment').length;
    ...
    return (
        <div>
            ...
            {!error && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-8 animate-enter">
                    {stats.map((stat) => (
                        ...
                        >
                            <span className="w-8 h-8 rounded-full flex items-center justify-center shrink-0" style={{ background: stat.iconBg }}>
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide mt-2" style={{ color: 'var(--blush-muted)' }}>
                                    {stat.dotColor && <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: stat.dotColor }} />}
                                    {stat.label}
                                </p>
                            </div>
                        </div>
            ...
            <nav className="flex items-center flex-wrap gap-6 mb-8 animate-enter" style={{ borderBottom: '1px solid var(--blush-border)' }}>
            ...
            {!error && list.length === 0 && (
                <div className="bg-white rounded-3xl p-14 card-bubble text-center animate-enter">
                    ...
                    <p className="font-bold mb-1" style={{ color: 'var(--blush-text)' }}>No orders {statusFilter ? 'with this status' : 'yet'}</p>
                    ...
                    {list.map((order, i) => (
                        ...
                        >
                            <div className="flex items-center justify-between gap-2">
                                <p className="font-bold tracking-tight text-[0.9375rem] truncate min-w-0 flex-1" style={{ color: 'var(--blush-text)' }}>{order.guest_name}</p>
                                <OrderStatusSelect orderId={order.id} status={order.status} />
                            </div>
                            <div className="flex items-center justify-between text-xs" style={{ color: 'var(--blush-muted)' }}>
                                <span>{order.guest_phone}</span>
                                <Link href={`/admin/orders/${order.id}`} className="font-semibold hover:text-[var(--blush-rose)] transition-colors" style={{ color: 'var(--blush-muted)' }}>
                                    #{order.id.slice(0, 8)}
                                </Link>
                            </div>
                            <div className="flex items-center justify-between text-xs pt-2 border-t" style={{ color: 'var(--blush-muted)', borderColor: 'var(--blush-border)' }}>
                                <span>{order.order_items?.[0]?.count ?? 0} item(s) · {new Date(order.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                                <span className="font-elegant-serif font-semibold text-sm" style={{ color: 'var(--blush-text)' }}>₹{order.subtotal}</span>
                            </div>
                        </div>
                    ))}
                    <div className="px-4 py-3 border-t text-xs" style={{ borderColor: 'var(--blush-border)', color: 'var(--blush-muted)', background: 'var(--blush-bg)' }}>
            ...
            {!error && list.length > 0 && (
                <div className="hidden lg:block bg-white rounded-3xl border overflow-x-auto animate-enter delay-200" style={{ borderColor: 'var(--blush-border)' }}>
                    <table className="w-full min-w-[820px] text-left text-sm">
                        <thead>
                            <tr style={{ background: 'var(--blush-bg)' }}>
                                <th className="px-5 py-3.5 font-bold text-[11px] uppercase tracking-wide" style={{ color: 'var(--blush-muted)' }}>Customer</th>
                                <th className="px-5 py-3.5 font-bold text-[11px] uppercase tracking-wide" style={{ color: 'var(--blush-muted)' }}>Order</th>
                                <th className="px-5 py-3.5 font-bold text-[11px] uppercase tracking-wide" style={{ color: 'var(--blush-muted)' }}>Items</th>
                                <th className="px-5 py-3.5 font-bold text-[11px] uppercase tracking-wide" style={{ color: 'var(--blush-muted)' }}>Total</th>
                                <th className="px-5 py-3.5 font-bold text-[11px] uppercase tracking-wide" style={{ color: 'var(--blush-muted)' }}>Date</th>
                                <th className="px-5 py-3.5 font-bold text-[11px] uppercase tracking-wide" style={{ color: 'var(--blush-muted)' }}>Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            {list.map((order, i) => (
                                ...
                                >
                                    <td className="px-5 py-4">
                                        <p className="font-bold tracking-tight text-[0.9375rem]" style={{ color: 'var(--blush-text)' }}>{order.guest_name}</p>
                                        <p className="text-xs mt-0.5" style={{ color: 'var(--blush-muted)' }}>{order.guest_phone}</p>
                                    </td>
                                    <td className="px-5 py-4">
                                        <Link
                                    ...
                                    <td className="px-5 py-4" style={{ color: 'var(--blush-text)' }}>{order.order_items?.[0]?.count ?? 0}</td>
                                    <td className="px-5 py-4">
                                        <span className="font-elegant-serif font-semibold" style={{ color: 'var(--blush-text)' }}>₹{order.subtotal}</span>
                                    </td>
                    ...
                    <div className="px-5 py-3 border-t text-xs" style={{ borderColor: 'var(--blush-border)', color: 'var(--blush-muted)', background: 'var(--blush-bg)' }}>
                        Showing {list.length} {list.length === 1 ? 'order' : 'orders'}
                    </div>
                </div>
            )}
