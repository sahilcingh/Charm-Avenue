...
function ProductChecklist({
    ...
    return (
        <div className="flex flex-col gap-1.5 max-h-56 overflow-y-auto rounded-2xl border p-3" style={{ borderColor: 'var(--blush-border)' }}>
            {products.length === 0 ? (
                <p className="text-xs" style={{ color: 'var(--blush-muted)' }}>No active products to choose from.</p>
            ) : (
                products.map((product) => (
                    <label key={product.id} className="flex items-center gap-2 text-sm cursor-pointer" style={{ color: 'var(--blush-text)' }}>
                        <input
                            type="checkbox"
                        ...
                        />
                        {product.name}
                        <span className="text-xs" style={{ color: 'var(--blush-muted)' }}>₹{product.price}</span>
                    </label>
                ))
            )}
        </div>
...
function ComboRow({ combo, products }: { combo: ComboWithProducts; products: ComboProductOption[] }) {
    ...
    const productNames = combo.productIds.map((id) => products.find((p) => p.id === id)?.name ?? 'Deleted product');
    ...
    return (
        <div className="flex flex-col gap-3 px-4 py-4 border-b last:border-0" style={{ borderColor: 'var(--blush-border)' }}>
            {editing ? (
                <>
                    <div>
                        <label className={labelClass} style={{ color: 'var(--blush-text)' }}>Name</label>
                        <input type="text" value={name} onChange={(e) => setName(e.target.value)} className={inputClass} style={{ color: 'var(--blush-text)' }} />
                    </div>
                    <div>
                        <label className={labelClass} style={{ color: 'var(--blush-text)' }}>Description (optional)</label>
                        <input type="text" value={description} onChange={(e) => setDescription(e.target.value)} className={inputClass} style={{ color: 'var(--blush-text)' }} />
                    </div>
                    <div>
                        <label className={labelClass} style={{ color: 'var(--blush-text)' }}>Discount %</label>
                        <input type="number" min={1} max={100} value={discountPercent} onChange={(e) => setDiscountPercent(e.target.value)} className={inputClass} style={{ color: 'var(--blush-text)' }} />
                    </div>
                    <div>
                        <label className={labelClass} style={{ color: 'var(--blush-text)' }}>Products (select at least 2)</label>
                ...
                <div className="flex items-start justify-between gap-3">
                    <div>
                        <div className="flex items-center gap-2 flex-wrap">
                            <p className="text-sm font-bold" style={{ color: 'var(--blush-text)' }}>{combo.name}</p>
                            <span className="badge-pill text-xs" style={{ background: 'var(--blush-border)', color: 'var(--blush-rose)' }}>
                                {combo.discount_percent}% off
                            </span>
                        </div>
                        {combo.description && (
                            <p className="text-xs mt-0.5" style={{ color: 'var(--blush-muted)' }}>{combo.description}</p>
                        )}
                        <p className="text-xs mt-1" style={{ color: 'var(--blush-muted)' }}>{productNames.join(' + ')}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                        <label className="flex items-center gap-1.5 cursor-pointer">
                            <span
                                className="relative inline-flex h-6 w-10 items-center rounded-full transition-colors duration-300"
                                style={{ background: combo.is_active ? 'var(--blush-rose)' : 'var(--blush-border)' }}
                            >
                                <span
                                ...
                                <input
                                    ...
                                    onChange={(e) => startTransition(() => toggleComboActive(combo.id, e.target.checked))}
                                    className="sr-only"
                                />
                            </span>
                        </label>
...
export default function ComboManager({ combos, products }: { combos: ComboWithProducts[]; products: ComboProductOption[] }) {
    const [isPending, startTransition] = useTransition();
    ...
    return (
        <div className="flex flex-col gap-6">
            <form onSubmit={handleCreate} className="bg-white rounded-3xl p-6 md:p-8 border flex flex-col gap-4" style={{ borderColor: 'var(--blush-border)' }}>
                <h2 className="font-elegant-serif text-lg" style={{ color: 'var(--blush-text)' }}>Add a Combo</h2>
                <div>
                    <label className={labelClass} style={{ color: 'var(--blush-text)' }}>Name</label>
                    <input
                        type="text"
