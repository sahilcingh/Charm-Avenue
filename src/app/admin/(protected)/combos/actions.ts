...
export async function createCombo(formData: FormData) {
    ...
    const { data: inserted, error } = await supabase
        .from('combos')
        .insert({ name: values.name, description: values.description, discount_percent: values.discountPercent })
        .select('id')
        .single();
    ...
    const { error: linkError } = await supabase
        .from('combo_products')
        .insert(values.productIds.map((productId) => ({ combo_id: inserted.id, product_id: productId })));
...
export async function updateCombo(comboId: string, formData: FormData) {
    ...
    const { error } = await supabase
        .from('combos')
        .update({ name: values.name, description: values.description, discount_percent: values.discountPercent })
        .eq('id', comboId);
    if (error) throw new Error(error.message);

    const { error: deleteError } = await supabase.from('combo_products').delete().eq('combo_id', comboId);
