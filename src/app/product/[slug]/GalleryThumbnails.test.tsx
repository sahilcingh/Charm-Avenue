...
describe('GalleryThumbnails', () => {
  it('renders nothing when there is only one (or no) image — no point showing a thumbnail strip for a single photo', () => {
    const { container } = render(
      <GalleryThumbnails images={[{ url: '/only.jpg', alt: 'Only' }]} activeIndex={0} />
    );
    expect(container.firstChild).toBeNull();
  });

  it('calls onSelectIndex when a plain (no color) thumbnail is clicked', () => {
  ...
  it(
    'is swipeable on touch devices the same way other horizontal scrollers in the app already ' +
      'work (failure case reported live: the row felt "static" and would not slide to reveal ' +
      'more thumbnails on a phone — it was missing the same scroll-snap/touch-momentum classes ' +
      'used elsewhere, e.g. the homepage Instagram carousel)',
    () => {
      const { container } = render(
        <GalleryThumbnails
          images={[
  ...
  it('applies whatever className the caller passes (e.g. md:hidden / hidden md:flex, to toggle which of the two rendered copies shows at a given breakpoint)', () => {
    const { container } = render(
      <GalleryThumbnails
        images={[
          { url: '/photo-1.jpg', alt: 'Photo 1' },
