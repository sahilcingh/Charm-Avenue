...
function CartProbe() {
  ...
  return (
    <div data-testid="probe">
      {itemCount}:{JSON.stringify(lines)}
    </div>
  );
}
...
describe('AddToCartButton', () => {
  ...
  it('includes the selected variant id in the cart line when provided', async () => {
    ...
    expect(await screen.findByTestId('probe')).toHaveTextContent(
      '1:[{"productId":"p1","variantId":"variant-red","quantity":1}]'
    );
  });
  ...
  it('includes the personalization text in the cart line when provided', async () => {
    ...
    expect(await screen.findByTestId('probe')).toHaveTextContent(
      '1:[{"productId":"p1","personalizationText":"For Priya","quantity":1}]'
    );
  });
  ...
  it('blocks adding to cart and shows an error when personalization is required but left blank', async () => {
    ...
    expect(
      await screen.findByText(/please fill in the personalization field/i)
    ).toBeInTheDocument();
