...
export default function ProductDetailInteractive({
    ...
    const resolved = resolveVariantDisplay({ price, originalPrice, image: galleryImages[0]?.url ?? '' }, selectedVariant);
    const showDiscount = Boolean(resolved.originalPrice) && isSaleWindowActive(saleStartsAt, saleEndsAt, new Date());
    const discountPct = showDiscount && resolved.originalPrice
    ...
    const lowStock =
        effectiveStockStatus === 'in_stock' && effectiveStockCount != null && effectiveStockCount <= (lowStockThreshold ?? 2) && effectiveStockCount > 0;
            ...
            <div className="flex flex-col">
                    ...
                    <div className="flex items-center gap-0.5">
                        {Array.from({ length: 5 }).map((_, i) => (
                            <Icon
                                ...
                                size={16}
                                variant="solid"
                                style={{ color: i < Math.round(rating) ? 'var(--blush-rose)' : 'var(--blush-border)' }}
                            />
                        ))}
                    </div>
                ...
                {(colors.length > 0 || sizes.length > 0) && (
                    <div className="flex flex-col gap-3 mb-5">
                        {colors.length > 0 && (
                            <div>
                                <p className="text-xs font-bold uppercase tracking-wide mb-1.5" style={{ color: 'var(--blush-muted)' }}>
                ...
                {personalizationEnabled && (
                    <div className="mb-5">
                        <label className={'text-xs font-bold uppercase tracking-wide mb-1.5 block'} style={{ color: 'var(--blush-muted)' }}>
                            {personalizationLabel || 'Personalization'}{personalizationRequired ? ' *' : ' (optional)'}
                        </label>
                        <input
                            type="text"
                ...
                <div className="flex items-center flex-wrap gap-3 mb-2">
                    <span className="font-elegant-serif font-bold text-3xl" style={{ color: 'var(--blush-rose)' }}>₹{resolved.price}</span>
                    {showDiscount && (
                        <>
                            <span className="text-lg line-through" style={{ color: 'var(--blush-muted)' }}>₹{resolved.originalPrice}</span>
                ...
                <div className="mb-6">
                    ...
                    {!hasVariants && stockStatus === 'made_to_order' && madeToOrderLeadTime && (
                        <span className="block text-xs mt-1.5" style={{ color: 'var(--blush-muted)' }}>{madeToOrderLeadTime}</span>
                    )}
                </div>

                <p className="text-base leading-relaxed mb-8" style={{ color: 'var(--blush-text)', opacity: 0.8 }}>{description}</p>
