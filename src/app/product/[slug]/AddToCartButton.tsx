...
export default function AddToCartButton({
    ...
    const handleClick = () => {
        ...
        setError(null);
        addToCart(productId, 1, { variantId, personalizationText: personalizationText?.trim() || undefined });
        setAdded(true);
    ...
    return (
        <div>
            <button
                ...
                style={{ background: added ? 'var(--blush-text)' : 'var(--blush-rose)', boxShadow: '0 4px 20px rgba(232,130,143,0.4)' }}
            >
                <Icon name={added ? 'CheckIcon' : 'ShoppingBagIcon'} size={18} />
                {added ? 'Added to Bag' : 'Add to Bag'}
            </button>
            {error && <p className="text-xs font-medium mt-2" style={{ color: 'var(--blush-rose-dark)' }}>{error}</p>}
        </div>
    );
}
