...
vi.mock('@/lib/supabase/client', () => ({
    createClient: () => ({
        ...
        from: (table: string) => {
            if (table === 'products') {
                return { select: () => ({ eq: () => ({ in: (_col: string, ids: string[]) => Promise.resolve({ data: ids.map((id) => ({ id })) }) }) }) };
            }
            if (table === 'profiles') {
...
describe('ProductCard', () => {
    ...
    it('saves the product to the wishlist when a logged-in customer clicks the heart', async () => {
        ...
        await waitFor(() => expect(insertMock).toHaveBeenCalledWith({ user_id: 'user-1', product_id: 'p1' }));
    ...
    it('shows an error toast and leaves the heart state unchanged if the wishlist write fails (failure case)', async () => {
        ...
        expect(await screen.findByText('Could not update your wishlist. Please try again.')).toBeInTheDocument();
